...
int VulkanVideo(char** args, size_t len)
{
#if 0
    char* bitstream_filename = args[0];

    u8 nalu_start_code[4] = { 0, 0, 0, 1 };

    util::sized_buffer bitstream = util::ReadWholeBinaryFileIntoMemory(bitstream_filename);
    u8* bitstream_bytes = new u8[bitstream.len + 4];
    memcpy(bitstream_bytes, bitstream.bytes, bitstream.len);
    memcpy(bitstream_bytes + bitstream.len, &nalu_start_code[0], 4);
#endif
...
// create a device
    vvb::init_vulkan();
    vvb::video_device_preferences p;
    p.required_codecs.push_back(vvb::Codec::H264);
	// parse bitstream
...
int main(int argc, char** argv)
{
    ...
    if (!strcmp(argv[1], "-parse"))
        return ParseBitstream(toolArgs, argc - 2);
    else if (!strcmp(argv[1], "-decode"))
        return DecodeH264(toolArgs, argc - 2);
    else if (!strcmp(argv[1], "-encode"))
        return EncodeBitstream(toolArgs, argc - 2);
#endif
#if USE_VULKAN
    else if (!strcmp(argv[1], "-vulkan"))
        return VulkanVideo(toolArgs, argc - 2);
#endif
    else
	goto usage;

usage:
    XERROR(0, "USAGE: %s -parse|-decode|-encode|-vulkan\n", argv[0]);
}
